...
static uint8 ledState;                 // LED state at last set/clr/blink update

#ifdef POWER_SAVING
static uint8 sleepLedState;            // Saved state of the LEDs
#endif
...
#if (HAL_LED == TRUE)
  static HalLedControl_t HalLedStatusTable[HAL_LED_DEFAULT_MAX_LEDS];   // LED Status Table
#endif /* HAL_LED */
...
{
  HalLedSet( sleepLedState, HAL_LED_MODE_ON );
}
#endif //POWER_SAVING
