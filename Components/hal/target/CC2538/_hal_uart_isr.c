...
void HalUARTInitIsr(void)
{
   ...
   */
  GPIOPinTypeUARTOutput(GPIO_B_BASE, GPIO_PIN_0);
  GPIOPinTypeUARTInput(GPIO_D_BASE, GPIO_PIN_3);
...
uint8 HalUARTOpenIsr(uint8 port, halUARTCfg_t *config)
{
  ...
  UARTFIFOLevelSet(HAL_UART_PORT, UART_FIFO_TX1_8, UART_FIFO_RX7_8);
  UARTFIFOEnable(HAL_UART_PORT);

  UARTIntClear(HAL_UART_PORT, (UART_INT_RX | UART_INT_TX));
  UARTIntEnable(HAL_UART_PORT, (UART_INT_RX | UART_INT_TX));
...
{
  UARTIntClear(HAL_UART_PORT, UART_INT_RX);
  procRx();

  UARTIntClear(HAL_UART_PORT, UART_INT_TX);
  procTx();
}
